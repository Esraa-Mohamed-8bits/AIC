...
IMPORTANT: This model uses standard ImageNet pretrained weights from
torchvision. No task-specific fine-tuning is performed and no learned
checkpoint is required. Weights are downloaded automatically on first use.

Model stats (pretrained, no fine-tuning):
  Parameters : ~2.54 M
  FLOPs      : ~0.06 GFLOPs (per 256x256 patch)
...
class FeatureExtractor(nn.Module):
    ...
    Uses frozen ImageNet pretrained weights — no fine-tuning, no external
    checkpoint file required.

    Args:
        weights_path: Optional path to a fine-tuned state-dict (.pth).
                      When None (default) the ImageNet pretrained weights
                      are used directly — no download needed beyond torchvision.
    ...
    def __init__(
        ...
        # Build backbone with standard ImageNet pretrained weights
        ...
        # Optionally load fine-tuned weights (not used in competition submission)
...
if __name__ == "__main__":
    ...
    extractor = FeatureExtractor()  # loads ImageNet weights automatically
    ...
    print("No fine-tuned checkpoint loaded — using ImageNet pretrained weights.")
